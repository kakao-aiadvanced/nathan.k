...
os.environ['OPENAI_API_KEY'] = 'FILTERED'
tavily = TavilyClient(api_key='FILTERED')
...
# doc_splits RAG 데이터들의 각 청크 끝에 source : {{ URL }} 을 붙여서 출처 표기에 문제가 없도록 한다.
...
Please include the source of the retrieved document in your response.
...
def grade_documents(state):
    ...
    for d in documents:
        ...
        # Document not relevant
            ...
            relevance = "No"
            continue
    return {"documents": filtered_docs, "question": question, "relevance": relevance}
...
#
    ...
    web_results = "\n".join([d["content"] + "source: " + d["url"] + "\n" for d in docs])
    web_results = Document(page_content=web_results)
    if documents is not None:
        documents.append(web_results)
    else:
        documents = [web_results]
    return {"documents": documents, "question": question, "web_search_count": web_search_count + 1}
...
def check_hallucination(state):
    ...
    if grade == "yes":
        ...
        if grade == "yes":
            ...
            return {"documents": documents, "question": question, "generation": generation , "hallucination": "no"}
        else:
            print("---DECISION: GENERATION DOES NOT ADDRESS QUESTION---")
            return {"documents": documents, "question": question, "generation": "" , "hallucination": "yes", "hallucination_count": hallucination_count + 1}
    else:
        pprint("---DECISION: GENERATION IS NOT GROUNDED IN DOCUMENTS, RE-TRY---")
        return {"documents": documents, "question": question, "generation": "", "hallucination": "err"}
    
def decide_to_answer(state):
    print("---DECISION NEXT STEP---")
    hallucination = state["hallucination"]
    hallucination_count = state["hallucination_count"]

    if hallucination == "no":
        print("answer is LGTM!")
        return "answer"
    elif hallucination == "err":
        print("failed hallucination check.")
        return "exit"
    elif hallucination_count == 2:
        print("regenerated answer also have hallucination!")
        return "exit"
    else:
        print("Decided regeneration!")
        return "retry"
...
inputs = {"question": "What is prompt?", "web_search_count": 0, "hallucination_count": 0}
